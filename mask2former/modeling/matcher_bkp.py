...
class HungarianMatcherWeakSup(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            print('images_color_similarities memory cost:', (m3 - m2) / (1024 * 1024), 'MB')

            # project mask to x & y axis
            # masks_x: (num_ins, 1, H_pad/4, 1)->(num_ins, H_pad/4)
            # masks_y: (num_ins, 1, 1, W_pad/4)->(num_ins, W_pad/4)
            src_mask_x = out_mask.max(dim=3, keepdim=True)[0].flatten(1, 3)
            m4 = torch.cuda.memory_allocated()
            print('src_mask_x memory cost:', (m4 - m3) / (1024 * 1024), 'MB')

            src_mask_y = out_mask.max(dim=2, keepdim=True)[0].flatten(1, 3)
            m5 = torch.cuda.memory_allocated()
            print('src_mask_y memory cost:', (m5 - m4) / (1024 * 1024), 'MB')
            ...
            print('log_fg/bg memory cost:', (m6 - m5) / (1024 * 1024), 'MB')
            ...
            if self.point_sample:
                ...
                # get gt labels
                tgt_similarities = point_sample(
                    tgt_similarities,
                    point_coords.repeat(tgt_similarities.shape[0], 1, 1),
                    align_corners=False,
                )  # (num_gt, k**2-1, num_points)

                src_similarities = point_sample(
                    src_similarities,
                    point_coords.repeat(src_similarities.shape[0], 1, 1),
                    align_corners=False,
                )  # (num_query, k**2-1, num_points)
        ...
        mmax = torch.cuda.max_memory_allocated()
        print('Matcher max memory cost:', mmax / (1024 * 1024), 'MB\n')
...
class HungarianMatcherProjMask(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            tgt_box_mask_x = tgt_box_mask.max(dim=3, keepdim=True)[0].flatten(1, 3)
            tgt_box_mask_y = tgt_box_mask.max(dim=2, keepdim=True)[0].flatten(1, 3)
            with autocast(enabled=False):
                src_mask_x = src_mask_x.float()
                tgt_box_mask_x = tgt_box_mask_x.float()

                src_mask_y = src_mask_y.float()
                tgt_box_mask_y = tgt_box_mask_y.float()
