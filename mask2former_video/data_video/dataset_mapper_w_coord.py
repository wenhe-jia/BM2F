...
__all__ = ["YTVISDatasetWithCoordMapper"]
...
class YTVISDatasetWithCoordMapper:
    """
    ...
    def __init__(
        ...
        *,
        ...
        logger.info(f"[YTVISDatasetWithCoordMapper] Augmentations used in {mode}: {augmentations}")
    ...
    def __call__(self, dataset_dict):
        ...
        dataset_dict["image"] = []
        dataset_dict["matched_coords"] = []
        dataset_dict["instances"] = []
        dataset_dict["file_names"] = []
        for frame_ind, frame_idx in enumerate(selected_idx):
            ...
            ##### load matching coordinates calculated by DINOv2 feature #####
            if frame_ind != len(selected_idx) - 1:
                curr_fr_name = file_names[frame_idx].split('/')[-1].split('.')[0]
                next_fr_name = file_names[selected_idx[frame_ind + 1]].split('/')[-1].split('.')[0]
                coords_file_path = file_names[frame_idx].replace(
                    'JPEGImages', 'DINOv2_feats_vits_480s'
                ).replace(
                    '{}.jpg'.format(curr_fr_name), 'coords_{}_{}.txt'.format(curr_fr_name, next_fr_name)
                )
